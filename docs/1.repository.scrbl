...
Git里最为强悍的、用于操作提交对象的命令之一，是有着很不起眼的名字的rebase命令。从根本上讲，你开发的每一个分支都有一个或多个“基础提交（base commit）”：它们是分支开始生长的位置。下图是一个很典型的场景。注意箭头是往时间更早的方向指的，因为每个提交引用着自己的父提交，而非子提交。所以，图中D和Z两个提交代表着各自分支的头部：

@image["images/branching.png"]

在本例中，运行branch命令会列出两个“头部”：@code{D}和@code{Z}，而两个分支的共同祖先是@code{A}。@code{show-branch}命令展示的正是上图所示的信息：
...
现在新的@code{Z}已经包含@code{D}带过来的改动了，但现在出现了一个新的提交对象专门代表@code{Z}与@code{D}的合并：即当前表示为@code{Z}'的提交。这个提交没有添加任何新东西，仅仅表示完成了把@code{D}与@code{Z}合并到一起这件事。某种意义上说它是一个“元提交（meta-commit）@notes5{类似于元数据meta-data是“关于数据的数据”，这里说的“元提交”meta-commit可以理解为“关于提交的提交”---译者注}”，因为它的内容仅仅与代码仓库内部完成的事情有关，并不涉及到工作目录里任何新的工作。

不过，有一个办法可以把@code{Z}分支直接移栽到@code{D}上，有效地让它在时间轴上前移：用我们强大的rebase命令。下图是我们的目标：

@image["images/rebase.png"]

图示的形态最直接地表明了我们想要完成的事情：我们本地正在开发的@code{Z}分支，要让它变成是基于主分支@code{D}的最新版本演变而来的。所以该命令叫做rebase（变基），因它改变的是分支最早开始派生的基础提交位置。如果反复运行该命令，你会不断得到一些补丁，它们一定是主分支的最新版本的补丁，而且不会给你开发中的分支添加不必要的合并提交。下面是要运行的命令，请与前面的merge操作进行对比：
...
为什么该操作只适用于本地分支？因为你每次rebase，都有潜在的可能会改变该分支里的每一个提交。之前，@code{W}是基于@code{A}的，它里面只包含从@code{A}到@code{W}的变换。然而在rebase之后，@code{W}会被重写，以保证它包含所有从@code{D}到@code{W’}需要的变换。更甚，从@code{W}到@code{X}的变换也变了，因为刚才的@code{A+W+X}现在是@code{D+W’+X’}了---如此等等。假如该分支对其他人也可见，而且任何一个下游用户从@code{Z}上派生出了他自己的本地分支，那他们的分支现在还指向旧的@code{Z}，而不是新的@code{Z’}。

一般来说，应当遵循这个规则：当你有一个本地分支、且没有从该分支分出别的分支的时候，用rebase，其它情况都用merge。此外merge还在你要把本地分支@code{pull}进主分支的时候有用。

@(footnotes5)

@section[#:tag "interactive-rebasing"]{交互式rebase}

上面的例子里运行rebase的时候，它自动重写从@code{W}到@code{Z}的所有提交，以保证@code{Z}分支变成以提交@code{D}（即分支@code{D}的头部）为基准的分支。不过，该重写过程你是可以全权控制的。如果给@code{rebase}命令加上@code{-i}选项，系统会弹出一个编辑窗口，让你选择对本地@code{Z}分支的每个提交分别做什么操作：
...
@itemlist[
	...
	@item{
	@bold{挤压（squash）}---“挤压”一个提交将把它的内容“折叠”进它的前面一个提交里。可以挤压任意多次。还用上面的例子，如果你挤压所有提交（第一个除外，第一个必须被@bold{选中}后面才有@bold{挤压}的地方），那么你得到的新@code{Z}分支就只包含一个提交，从@code{D}长出来。适用于你的改动分散在多个提交里、但你希望改写提交历史让这些改动都放在一个提交里的情况。
	}
	@item{
...
上图可以这样来看：我们的开发主线是@code{D}，在@code{D}往前推三个提交的时候开启了一个尝试性的分支，即现在的@code{Z}。在这期间的某个时候，当时@code{C}和@code{X}分别是它们各自分支的头部，这时我们决定启动另一个尝试性分支，即后来的@code{L}。现在我们发现@code{L}的代码挺好的，但还不足以合并到主线上，所以我们决定把@code{L}上面的改动转移到正在开发的分支@code{Z}上，让整个改动看起来像是在同一个分支上完成的。噢，我们还想快速地编辑一下@code{J}，改一下版权日期就好，因为之前做改动的时候忘了时间是2008！下面的命令帮助我们解开这个结：
